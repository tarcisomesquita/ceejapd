...
';
  ...
  codigo = codigo + gerar_texto(linha);
...
function corrigir(token, linha, respostas) {
  const matricula = autentica_usuario(token);
  ...
  const matricula_g = String(geradas.getRange('GER!B' + linha).getDisplayValues());
  const resposta_g = String(geradas.getRange('GER!F' + linha).getDisplayValues());
